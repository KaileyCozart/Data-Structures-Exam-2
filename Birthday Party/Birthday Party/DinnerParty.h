...
public:
	...
	*/
	PartyTicket * add(std::string name) {
		...
		*/
		...
		std::list<std::string>::iterator &frontOfRoom = room.begin();
		DinnerPartyTicket* new_dinner_party_ticket = new DinnerPartyTicket(this, frontOfRoom);
		return new_dinner_party_ticket;
	}
	...
	*/
	void remove(std::list<std::string>::iterator it) {
		//FIXME 1c: Someone is getting out of hand. Let's ask them to leave.
		room.erase(it);
	}
};
#endif